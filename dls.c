/*****************************************************************************
 * Copyright (C) ANERI R.D. desaianeri650@gmail.com
 *
...
void usage(){
	printf("Run the code with ./project\n");
}
...
GtkWidget *aim, *oim, *nim, *in0im, *in1im, *hu, *hd, *outim, *outim_ch, *naim, *noim, *exorim, *no_out, *too_in, *comp_ckt;
...
static gboolean key_event(GtkWidget *widget, GdkEventKey *event){
	/*	close the window on pressing the escape button	*/
...
static void nand_dis(GtkWidget *widget, gpointer data){
	...
	if(nflag == 1){
		nflag = 0;
		if(q1 -> gtype == IN_0 || q1 -> gtype == IN_1){
	                err_win = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	                gtk_window_set_position(GTK_WINDOW(err_win), GTK_WIN_POS_CENTER);
	                ...
	                gtk_box_pack_start(GTK_BOX(qbox), quit,TRUE, TRUE, 0);
	                comp_ckt = gtk_image_new_from_file("comp_ckt_im.png");
	                gtk_box_pack_start(GTK_BOX(qbox), comp_ckt ,TRUE ,TRUE, 0);
	                gtk_widget_show(comp_ckt);
	                gtk_container_add(GTK_CONTAINER(err_win), qbox);
	                gtk_widget_show_all(err_win);

	                g_signal_connect_swapped(quit, "clicked", G_CALLBACK(gtk_widget_destroy), err_win);
	
//			printf("NAND GAte cannot be added to the input\n");
			return ;
}
}
                count = 0;
...
//                        printf("AND GAte cannot be added to the input\n");
                        return ;
}
}
		count = 0;
...
static void in0_dis(GtkWidget *widget, gpointer data){
	...
	GtkWidget *quit, *qbox, *err_win;
		...
		if(p -> gtype == NOT){
         ...
         if(count == 2){
                p = dequeue(&q);
                while((p -> gtype == IN_0 || p -> gtype == IN_1) && (!qempty(&q)))
                       p =  dequeue(&q);
		if((p -> gtype == IN_0 || p -> gtype == IN_1) && (qempty(&q))){
	                err_win = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	                gtk_window_set_position(GTK_WINDOW(err_win), GTK_WIN_POS_CENTER);
	                ...
	                gtk_box_pack_start(GTK_BOX(qbox), quit,TRUE, TRUE, 0);
	                too_in = gtk_image_new_from_file("too_in_im.png");
	                gtk_box_pack_start(GTK_BOX(qbox), too_in,TRUE, TRUE, 0);
	                gtk_widget_show(too_in);
	                gtk_container_add(GTK_CONTAINER(err_win), qbox);
	                gtk_widget_show_all(err_win);
...
int main( int argc, char *argv[]){
	...
	if(strcmp(argv[1], "-h") == 0){
		...
		printf("Option not available: Try -h\n");
}
